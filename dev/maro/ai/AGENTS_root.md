...
2. Blueprint와 Binding은 PR에 소유되지 않는 독립 설계 자원으로 유지한다.
3. 업무 변경은 기존 Binding ID를 유지하면서 변경 가능한 설계 필드와 해석 결과를 수정한다.
4. Binding이 고정한 Blueprint ID·key·version과 Aggregate 정체성은 같은 Binding 수정으로 바꾸지 않는다.
5. 다음 단계는 Binding이 아니라 Materialize된 기존 VM3를 소비한다.
6. Materialization 여부는 Binding 수정 가능 여부를 제한하지 않는다.
7. 새 PR의 Pit은 직전 최신 PR Pit의 Blueprint BindingSet 참조를 snapshot하여 동일 Binding ID들을 이어받는다.
...
특정 업무에 Blueprint를 적용하는 설계 작업 단위다. 입력, Profile, Logic, 대상 Contract, 검증 결과와 해석 결과를 가지며, 같은 업무 설계의 변경은 동일 Binding ID에서 갱신한다.

### MsBlueprintBindingSet

하나의 마이크로서비스 개발 범위에 속한 Aggregate, Move, Feature, Facade 및 선택적 Query/History/Event Binding을 묶는 교차 검증·Materialization 경계다. Pit은 이 독립 설계 자원을 생성 출처로 참조하지만, Set이 Pit이나 PR에 소유되지는 않는다.
...
3. Binding의 Draft/Validated/Resolved/Failed 수명주기를 사용하되, VM3 Materialization은 resolve 및 validation이 끝난 Binding만 사용한다.
4. 업무 설계를 수정할 때는 같은 Binding을 다시 resolve한다. 수정과 Pit 동기화를 함께 수행하는 문맥에서는 기존 해석 결과를 트랜잭션 내부 비교 기준으로 보관한다.
...
9. Materializer는 동일 Binding ID의 동일 해석 결과에 대해 멱등적이어야 한다.
...
27. Pit을 Blueprint로 생성할 때 `Pit.sourceMsBlueprintBindingSetId`에 전체 설계 묶음의 ID를 기록하고 snapshot과 projection에서 보존한다. Pit에 개별 Binding ID 출처 필드를 두지 않는다.
28. 업무 변경은 같은 Binding ID의 변경 가능한 설계 필드와 해석 결과를 수정한다. Pit 문맥의 수정은 수정 전·후 `ResolvedAggregateModel`을 비교하여 선택한 활성 Pit을 동기화하고, 카탈로그 문맥의 수정은 기존 Pit VM3 snapshot을 유지한다.
29. 같은 Binding 수정으로 `blueprintId`, Blueprint key/version 또는 `aggregateName`을 교체하지 않는다. 공통 Blueprint의 새 버전은 별도 자원이며 기존 Binding에 암묵 적용하지 않는다.
30. Aggregate, Move, Feature, Facade Binding은 서로의 물리 ID나 FK를 소유하지 않는다. `MsBlueprintBindingSet`이 타입별 Binding ID 목록을 소유하고, Binding 사이의 실행 의존성은 논리 ContractRef로 교차 검증한다.
31. 기존 Aggregate 단일 생성 API를 유지하더라도 Pit Materializer를 개별 Binding으로 직접 호출하지 않는다. 속한 BindingSet을 해석하거나 최초 Aggregate-only BindingSet을 생성한 뒤 Set 경계로 위임한다.
32. Binding이 BindingSet에 포함되었거나 이미 Materialize되었다는 이유로 Binding 수정을 거부하지 않는다. 카탈로그 수정은 같은 Binding ID의 설계만 갱신할 수 있고, Pit 문맥의 수정은 같은 Binding 갱신과 선택한 활성 Pit 동기화를 한 트랜잭션으로 수행한다.
33. 새 PR의 Pit snapshot은 임의의 과거 PR이 아니라 직전 최신 PR의 Pit/Pi 모델과 `sourceMsBlueprintBindingSetId`를 복사한다. Binding 또는 BindingSet Entity를 PR별로 복제하지 않으며, 복사된 Set 참조를 통해 동일 구성원 Binding ID를 계속 사용한다.
...
├─ featureBindingIds[]
├─ moveBindingIds[]
├─ queryModelBindingIds[]   // 필요한 경우
├─ historyBindingIds[]      // 필요한 경우
├─ eventBindingIds[]        // 필요한 경우
├─ validationResult
└─ status
```

`MsBlueprintBindingSet`은 하나의 마이크로서비스 개발 범위를 나타내는 원자적인 작업 결과와 교차 검증 경계다. Materialization된 하나의 Pit IR 전체가 이 Set을 설계 출처로 참조한다. 타입별 ID 목록은 Set의 구성원 참조이며 JPA FK 관계로 만들지 않는다. 각 Binding의 책임을 합친 거대한 Binding으로 구현하지 않는다.
...
Binding은 업무 설계의 동일한 정체성을 유지하면서 Draft부터 Resolved까지 상태가 변할 수 있다. VM3 생성과 재동기화에는 `Resolved` 상태만 사용한다.
...
여러 Binding을 한꺼번에 설계하거나 교차 검증하기 위한 작업 상태가 필요하면 다음처럼 별도 이름을 사용할 수 있다.

- `AggregateBlueprintBindingDraft`
- `GenoBindingCandidate`
- `BlueprintAuthoringSession`

별도 Candidate를 도입한 경우에는 영속 Binding과 동일 Repository와 조회 API에서 섞이지 않게 한다. Aggregate 단독 흐름에서는 기존 `AggregateBlueprintBinding`의 상태를 사용할 수 있다.
...
Materialization이 실패해도 확정 Binding은 보존하고 동일 Binding ID의 동일 해석 결과로 재시도할 수 있어야 한다.

Materialization 상태는 Binding의 편집 잠금 상태가 아니다. Binding은 Materialization 전후에 같은 ID로 수정할 수 있다. Pit 문맥이 없는 수정은 Binding 설계만 갱신하고 기존 Pit VM3 snapshot을 자동 변경하지 않으며, Pit 문맥의 수정은 선택한 활성 Pit 동기화까지 포함한다.
...
`BlueprintMaterializationEntry.lineageId`는 생성된 VM3 결과를 역추적하기 위한 값이다. 이를 Blueprint, Binding 또는 `ResolvedAggregateModel`의 입력이나 소유 필드로 역전파하지 않는다. 동일 Binding이 다시 Materialize된 경우에는 materializationId와 생성 시각으로 실행 결과를 구분한다.
...
- 업무 변경 시 기존 Binding ID를 유지하고 변경 가능한 설계 필드와 해석 결과를 수정한다.
- Pit 동기화가 포함된 수정은 직전의 `ResolvedAggregateModel`을 트랜잭션 내부 비교 기준으로 보관한다.
- Pit 동기화가 포함된 수정은 전·후 해석 결과를 비교하여 Create, Update, Remove 계획을 만든다.
- 자동 생성 영역과 사용자 구현 영역을 구분한다.
- 사용자 Move 및 확장 Logic은 재생성 시 보존한다.
- Materialize된 VM3의 직접 수정은 원칙적으로 금지한다.
- 기존 API가 VM3 직접 수정을 요구하면 변경을 같은 Binding의 설계 변경으로 역투영하거나 명시적인 호환 모드로 격리한다.

### 8.5 PR과 동일 Binding 수정

PR은 Blueprint 또는 Binding의 소유자가 아니다. 새 PR의 Pit snapshot은 직전 최신 PR의 Pit/Pi 모델과 `sourceMsBlueprintBindingSetId`를 복사하고, 그 Set에 포함된 동일 Binding ID들을 계속 사용한다.

```text
새 PR 생성
→ 직전 최신 PR의 Pit/Pi snapshot 복사
→ 직전 최신 Pit의 sourceMsBlueprintBindingSetId 복사
→ 같은 BindingSet의 동일 Binding ID 재사용

새 PR + 이후 업무 설계 변경 있음
→ 기존 Binding의 이전 ResolvedAggregateModel을 트랜잭션 내부에 보관
→ 동일 Blueprint 버전으로 다시 resolve
→ 같은 Binding ID의 변경 가능한 필드와 해석 결과 수정
→ 수정 전·후 ResolvedAggregateModel diff
→ Binding 수정과 활성 Pit 동기화를 한 트랜잭션으로 수행
→ 기존 sourceMsBlueprintBindingSetId 유지
```

Aggregate의 정체성이나 Blueprint 선택 자체가 바뀌는 변경은 같은 Binding 수정으로 숨기지 않는다. 이 경우에만 새 논리 Binding 또는 명시적인 Blueprint migration이 필요한지 판단한다.
...
- 이름만 동일한 클래스를 자동으로 동일 Contract라고 판단하지 않는다. lineage, Binding의 StageEntity version과 semantic key를 확인한다.
...
- 동일 Binding ID의 동일 해석 결과를 재실행해도 중복 생성되지 않는다.
- 생성된 모든 VM3가 Binding으로 역추적된다.
- 동일 Binding의 업무 설계 변경을 적용해도 유지 대상 VM3의 lineage가 유지된다.
- 실패 후 재시도가 가능하다.
...
6. 동일 Binding의 수정 전·후 `ResolvedAggregateModel` diff 테스트
...
12. Pit 생성, snapshot과 projection에서 `sourceMsBlueprintBindingSetId`가 보존되는지 검증하는 테스트
13. BindingSet 포함 및 Materialization 이력과 무관하게 같은 Binding ID가 수정되는지 검증하는 테스트
14. 새 PR의 Pit이 직전 최신 PR Pit의 `sourceMsBlueprintBindingSetId`를 복사하는지 검증하는 테스트
...
- 이 모델은 Blueprint 정의인가, 업무별 Binding인가, 별도 작업 중인 DesignWork인가?
...
- 재생성과 동일 Binding 수정 시 사용자 구현이 보존되는가?
- 모든 산출물의 lineage와 source Binding을 추적할 수 있는가?
- 실패 시 Binding을 이용해 안전하게 재시도할 수 있는가?
- 업무 변경이 없는 새 PR이 기존 Binding ID를 재사용하는가?
- 새 PR의 Pit이 직전 최신 PR Pit의 BindingSet 참조를 snapshot했는가?
- Materialization 여부를 Binding 편집 가능 여부로 사용하지 않는가?
- Pit이 전체 Geno 설계 출처인 BindingSet ID를 보존하는가?
- 업무 변경이 같은 Binding ID의 허용된 설계 필드 수정으로 표현되는가?
- 같은 Binding 수정 과정에서 Blueprint ID·key·version과 Aggregate 정체성이 유지되는가?
...
이 문서가 이미 명시적으로 통합한 충돌은 동일 Binding ID 수정과 Blueprint 정체성 고정 규칙을 따른다. 그 밖에 사용자 요청, 현재 코드, 모듈별 구현 또는 참조 문서가 충돌하면 추측으로 해결하지 않는다. 충돌하는 필드, 책임, 버전 또는 의존 방향을 구체적으로 제시하고 사용자에게 기준을 확인한다.
